...
#include <gradientstops_widget.h>
...
void QtGradientStopsWidget::mousePressEvent(QMouseEvent *e)
{
   ...
   if (stop) {
      if (e->modifiers() & Qt::ControlModifier) {
         ...
         if (oldCurrent) {
            auto stops = d_ptr->m_model->stops();
...
void QtGradientStopsWidget::mouseMoveEvent(QMouseEvent *e)
{
   ...
   if (!d_ptr->m_moveStops.isEmpty()) {
      ...
      bool first  = true;
      auto itStop = d_ptr->m_moveStops.constBegin();
      ...
      bool forward = true;
      auto itNewPos = newPositions.constBegin();
      ...
      auto itOld = d_ptr->m_moveOriginal.constBegin();
...
void QtGradientStopsWidget::paintEvent(QPaintEvent *e)
{
   ...
   QColor back = QColor::fromRgb((back1.red() + back2.red()) / 2,
         (back1.green() + back2.green()) / 2, (back1.blue() + back2.blue()) / 2);
...
void QtGradientStopsWidget::slotStopAdded(QtGradientStop *stop)
{
   Q_D(QtGradientStopsWidget);
   d->slotStopAdded(stop);
}

void QtGradientStopsWidget::slotStopRemoved(QtGradientStop *stop)
{
   Q_D(QtGradientStopsWidget);
   d->slotStopRemoved(stop);
}

void QtGradientStopsWidget::slotStopMoved(QtGradientStop *stop, qreal newPos)
{
   Q_D(QtGradientStopsWidget);
   d->slotStopMoved(stop, newPos);
}

void QtGradientStopsWidget::slotStopsSwapped(QtGradientStop *stop1, QtGradientStop *stop2)
{
   Q_D(QtGradientStopsWidget);
   d->slotStopsSwapped(stop1, stop2);
}

void QtGradientStopsWidget::slotStopChanged(QtGradientStop *stop, const QColor &newColor)
{
   Q_D(QtGradientStopsWidget);
   d->slotStopChanged(stop, newColor);
}

void QtGradientStopsWidget::slotStopSelected(QtGradientStop *stop, bool selected)
{
   Q_D(QtGradientStopsWidget);
   d->slotStopSelected(stop, selected);
}

void QtGradientStopsWidget::slotCurrentStopChanged(QtGradientStop *stop)
{
   Q_D(QtGradientStopsWidget);
   d->slotCurrentStopChanged(stop);
}

void QtGradientStopsWidget::slotNewStop()
{
   Q_D(QtGradientStopsWidget);
   d->slotNewStop();
}

void QtGradientStopsWidget::slotDelete()
{
   Q_D(QtGradientStopsWidget);
   d->slotDelete();
}

void QtGradientStopsWidget::slotFlipAll()
{
   Q_D(QtGradientStopsWidget);
   d->slotFlipAll();
}

void QtGradientStopsWidget::slotSelectAll()
{
   Q_D(QtGradientStopsWidget);
   d->slotSelectAll();
}

void QtGradientStopsWidget::slotZoomIn()
{
   Q_D(QtGradientStopsWidget);
   d->slotZoomIn();
}

void QtGradientStopsWidget::slotZoomOut()
{
   Q_D(QtGradientStopsWidget);
   d->slotZoomOut();
}

void QtGradientStopsWidget::slotResetZoom()
{
   Q_D(QtGradientStopsWidget);
   d->slotResetZoom();
}

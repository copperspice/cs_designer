...
QMap<const QtProperty *, QtProperty *> *propertyToWrappedProperty()
{
   static QMap<const QtProperty *, QtProperty *> retval;
   return &retval;
}
...
/*    broom - put back when QVariant operator= resolved
      emit q_ptr->attributeChanged(varProp, m_enumIconsAttribute, QVariant::fromValue(iconMap));
*/
...
{
   const QMap<const QtProperty *, QPair<QtVariantProperty *, int>>::const_iterator iter =
      d_ptr->m_propertyToType.constFind(property);

   if (iter == d_ptr->m_propertyToType.constEnd()) {
      return nullptr;
   }
...
QVariant QtVariantPropertyManager::value(const QtProperty *property) const
{
   ...
   } else if (QtSizePolicyPropertyManager *sizePolicyManager = dynamic_cast<QtSizePolicyPropertyManager *>(manager)) {

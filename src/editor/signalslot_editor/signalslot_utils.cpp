...
static void memberList(QDesignerFormEditorInterface *core, QObject *object,
   qdesigner_internal::MemberType member_type, bool showAll, SignaturePredicate predicate, OutputIterator it)
{
   if (! object) {
      return;
   }
   ...
   const qdesigner_internal::WidgetDataBase *wdb = dynamic_cast<qdesigner_internal::WidgetDataBase *>(core->widgetDataBase());
   if (!wdb) {
      return;
   }
   ...
   qdesigner_internal::MetaDataBase *metaDataBase = dynamic_cast<qdesigner_internal::MetaDataBase *>(core->metaDataBase());
   if (!metaDataBase) {
      return;
   }
...
SignalMatchesSlotPredicate::SignalMatchesSlotPredicate(QDesignerFormEditorInterface *core, const QString &peer,
   qdesigner_internal::MemberType memberType) :
   m_peer(peer),
...
class ReverseClassesMemberIterator
{
 public:
   ...
   ReverseClassesMemberIterator &operator*()     {
      return *this;
   }
   ReverseClassesMemberIterator &operator++(int) {
      return *this;
   }
   ReverseClassesMemberIterator &operator++()    {
      return *this;
...
static inline bool truePredicate(const QString &)
{
   return true;
}
...
QMap<QString, QString> getMatchingSlots(QDesignerFormEditorInterface *core, QObject *object, const QString &signalSignature,
   bool showAll)
{
   QMap<QString, QString> rc;

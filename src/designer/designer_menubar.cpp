...
ActionList QDesignerMenuBar::contextMenuActions()
{
   ...
   if (QAction *action = safeActionAt(m_currentIndex)) {
      ...
      if (! dynamic_cast<SpecialMenuAction *>(action)) {
         ...
         QAction *remove_action = new QAction(tr("Remove Menu '%1'").formatArg(action->menu()->objectName()), nullptr);
...
void QDesignerMenuBar::leaveEditMode(LeaveEditMode mode)
{
   ...
   if (m_currentIndex >= 0 && m_currentIndex < realActionCount()) {
      ...
      const QString niceObjectName = ActionEditor::actionTextToName(m_editor->text(), QString("menu"));

      QMenu *menu = dynamic_cast<QMenu *>(fw->core()->widgetFactory()->createWidget(QString("QMenu"), this));
...
void QDesignerMenuBar::adjustIndicator(const QPoint &pos)
{
   ...
   if (pos != QPoint(-1, -1)) {
      QDesignerMenu *m = dynamic_cast<QDesignerMenu *>(action->menu());
      if (!m || m->parentMenu()) {
...
void QDesignerMenuBar::dropEvent(QDropEvent *event)
{
   ...
   if (const ActionRepositoryMimeData *d = dynamic_cast<const ActionRepositoryMimeData *>(event->mimeData())) {
...
void QDesignerMenuBar::hideMenu(int index)
{
   ...
   if (action && action->menu()) {
      ...
      if (QDesignerMenu *menu = dynamic_cast<QDesignerMenu *>(action->menu())) {
         menu->closeMenuChain();
      }
   }
}
...
{
   if (action && !dynamic_cast<SpecialMenuAction *>(action)) {

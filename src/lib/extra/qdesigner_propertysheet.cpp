...
class QDesignerPropertySheetPrivate
{
 public:
   ...
   QVariant defaultResourceProperty(int index)
   const; // of type QPixmap / QIcon (maybe it can be generalized for all types, not resource only)
...
/*
    Resource properties are those which:
        ...
        3) we don't store them in the meta object system (because designer keeps different data structure for them)
*/
...
QDesignerPropertySheetPrivate::QDesignerPropertySheetPrivate(QDesignerPropertySheet *sheetPublic,
   ...
   : q(sheetPublic), m_core(formEditorForObject(sheetParent)), m_meta(m_core->introspection()->metaObject(object)),
     ...
     m_object(object), m_lastLayout(0), m_lastLayoutPropertySheet(0), m_LastLayoutByDesigner(false),
     m_pixmapCache(0), m_iconCache(0)
{
}
   ...
   : QObject(parent), d(new QDesignerPropertySheetPrivate(this, object, parent))
{
   ...
   if (object->isWidgetType()) {
      ...
      if (dynamic_cast<const QToolBar *>(d->m_object.data())) {
         ...
         createFakeProperty(QString("floatable"), QVariant(true));
...
{
   // used internally
   if (propName == QString("database") ||
      propName == QString("buttonGroupId")) {
      return false;
   }
   ...
   if (!QDesignerPropertySheet::internalDynamicPropertiesEnabled() && propName.startsWith(QString("_q_"))) {
      return false;
   }
...
bool QDesignerPropertySheet::reset(int index)
{
   ...
   if (d->isStringProperty(index)) {
      ...
      if (propertyName(index) == QString("objectName")) {

...
void RCC_Support::write(const char *str, int len)
{
...
void RCC_Support::writeByteArray(const QByteArray &other)
{
   m_out.append(other);
}
...
class RCCFileInfo
{
 ...
 public:
   qint64 writeDataBlob(RCC_Support &lib, qint64 offset, QString &errorMessage);
   qint64 writeDataName(RCC_Support &, qint64 offset);
   void writeDataInfo(RCC_Support &lib);
...
void RCCFileInfo::writeDataInfo(RCC_Support &lib)
{
   const bool text = (lib.m_format == RCC_Support::C_Code);
...
qint64 RCCFileInfo::writeDataBlob(RCC_Support &lib, qint64 offset, QString &errorMessage)
{
   const bool text = (lib.m_format == RCC_Support::C_Code);
...
qint64 RCCFileInfo::writeDataName(RCC_Support &lib, qint64 offset)
{
   const bool text = (lib.m_format == RCC_Support::C_Code);
...
bool RCC_Support::interpretResourceFile(QIODevice *inputDevice, const QString &fname, QString currentPath, bool ignoreErrors)
{
...
bool RCC_Support::addFile(const QString &alias, const RCCFileInfo &file)
{
   Q_ASSERT(m_errorDevice);
...
bool RCC_Support::readFiles(bool ignoreErrors, QIODevice &errorDevice)
{
   reset();
...
QStringList RCC_Support::dataFiles() const
{
   QStringList retval;
...
// Determine resource identifier (':/newPrefix/images/p1.png') to file via recursion
static void resourceFile_Recursion(const RCCFileInfo *m_root, const QString &path, RCC_Support::ResourceFile &m)
{
   const QChar slash = '/';
...
RCC_Support::ResourceFile RCC_Support::resourceFile() const
{
   ResourceFile rc;
...
bool RCC_Support::output(QIODevice &outDevice, QIODevice &errorDevice)
{
   m_errorDevice = &errorDevice;
...
void RCC_Support::writeHex(quint8 tmp)
{
...
void RCC_Support::writeNumber1(quint8 number)
{
   if (m_format == RCC_Support::Binary) {
      writeChar(number);
...
void RCC_Support::writeNumber2(quint16 number)
{
   if (m_format == RCC_Support::Binary) {
      writeChar(number >> 8);
      writeChar(number);
...
void RCC_Support::writeNumber4(quint32 number)
{
   if (m_format == RCC_Support::Binary) {
      writeChar(number >> 24);
      writeChar(number >> 16);
...
bool RCC_Support::writeDataBlobs()
{
   Q_ASSERT(m_errorDevice);
...
bool RCC_Support::writeDataNames()
{
   if (m_format == C_Code) {
...
bool RCC_Support::writeDataStructure()
{
   if (m_format == C_Code) {
...
bool RCC_Support::writeInitializer()
{
   if (m_format == C_Code) {

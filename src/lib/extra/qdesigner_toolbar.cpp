...
ActionList ToolBarEventFilter::contextMenuActions(const QPoint &globalPos)
{
   ...
   if (action && index != 0 && !action->isSeparator()) {
      QAction *newSeperatorAct = new QAction(tr("Insert Separator before '%1'").formatArg(action->objectName()), 0);
   ...
   // Remove
   if (action) {
      QAction *a = new QAction(tr("Remove action '%1'").formatArg(action->objectName()), 0);
      itemData.setValue(action);
      a->setData(itemData);
   ...
   QAction *remove_toolbar = new QAction(tr("Remove Toolbar '%1'").formatArg(m_toolBar->objectName()), 0);

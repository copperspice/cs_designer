...
void Connection::setSource(QObject *source, const QPoint &pos)
{
   ...
   m_source = source;
   if (QWidget *widget = dynamic_cast<QWidget *>(source)) {
      m_source_pos = pos;
...
void Connection::setTarget(QObject *target, const QPoint &pos)
{
   ...
   m_target = target;
   if (QWidget *widget = dynamic_cast<QWidget *>(target)) {
      m_target_pos = pos;
...
void ConnectionEdit::paintEvent(QPaintEvent *e)
{
   ...
   for (QWidget *w : heavy_highlight_set) {
   ...
   for (QWidget *w : light_highlight_set) {
      p.drawRect(fixRect(widgetRect(w)));
   }
...
void ConnectionEdit::mouseMoveEvent(QMouseEvent *e)
{
   ...
   switch (state()) {
      ...
      case Editing:
         if ((e->buttons() & Qt::LeftButton) && m_start_connection_on_drag
...
static ConnectionEdit::ConnectionSet findConnectionsOf(const ConnectionEdit::ConnectionList &cl, ObjectIterator oi1,
...
{
   for (Connection *con : m_sel_con_set) {
      con->update();
   }
...
void ConnectionEdit::addConnectionX(Connection *con)
{

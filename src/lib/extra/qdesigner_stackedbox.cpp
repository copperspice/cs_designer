...
QStackedWidgetPreviewEventFilter::QStackedWidgetPreviewEventFilter(QStackedWidget *parent) :
   ...
   m_prev(createToolButton(m_stackedWidget, Qt::LeftArrow,  QString("__qt__passive_prev"))),
   m_next(createToolButton(m_stackedWidget, Qt::RightArrow, QString("__qt__passive_next")))
{
...
{
   if (o == m_prev) {
      const QString msg = tr("Go to previous page of %1 '%2' (%3/%4).").formatArg(stackedClassName(m_stackedWidget)).formatArg(
            m_stackedWidget->objectName()).formatArg(m_stackedWidget->currentIndex() + 1).formatArg(m_stackedWidget->count());
      m_prev->setToolTip(msg);
   } else {
      if (o == m_next) {
         const QString msg = tr("Go to next page of %1 '%2' (%3/%4).").formatArg(stackedClassName(m_stackedWidget)).formatArg(
               m_stackedWidget->objectName()).formatArg(m_stackedWidget->currentIndex() + 1).formatArg(m_stackedWidget->count());
